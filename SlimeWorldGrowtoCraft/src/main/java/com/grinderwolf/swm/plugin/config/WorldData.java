...
import static com.grinderwolf.swm.plugin.config.ConfigManager.getDatasourcesConfig;
...
public class WorldData {
    ...
    @Setting("source")
    private String dataSource = getDatasourcesConfig().getWorldConfig().getDataSource();
    @Setting("spawn")
    private String spawn = getDatasourcesConfig().getWorldConfig().getSpawn();
    @Setting("difficulty")
    private String difficulty = getDatasourcesConfig().getWorldConfig().getDifficulty();
    @Setting("allowMonsters")
    private boolean allowMonsters = getDatasourcesConfig().getWorldConfig().isAllowMonsters();
    @Setting("allowAnimals")
    private boolean allowAnimals = getDatasourcesConfig().getWorldConfig().isAllowAnimals();
    @Setting("dragonBattle")
    private boolean dragonBattle = getDatasourcesConfig().getWorldConfig().isDragonBattle();
    @Setting("pvp")
    private boolean pvp = getDatasourcesConfig().getWorldConfig().isPvp();
    @Setting("environment")
    private String environment = getDatasourcesConfig().getWorldConfig().getEnvironment();
    @Setting("worldType")
    private String worldType = getDatasourcesConfig().getWorldConfig().getWorldType();
    @Setting("defaultBiome")
    private String defaultBiome = getDatasourcesConfig().getWorldConfig().getDefaultBiome();
    @Setting("loadOnStartup")
    private boolean loadOnStartup = getDatasourcesConfig().getWorldConfig().isLoadOnStartup();
    @Setting("readOnly")
    private boolean readOnly = getDatasourcesConfig().getWorldConfig().isReadOnly();
